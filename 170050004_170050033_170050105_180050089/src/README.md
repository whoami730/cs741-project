...
However, this speedup comes at a cost, the lattice attack is not guaranteed to succeed every time and it requires somewhat more information than predicted by the information-theoretic lower bound. Thus, it may not perform well in cases where the number of outputs are just enough with respect to the bit-sizes, in order to predict the initial seed. However the SMT-based attack can, even in these cases, predict the initial seed correctly! And not just that, it is also able to recover the unknown parameters of the (truncated) LCG being used, including any possible multiple solutions as usual!
